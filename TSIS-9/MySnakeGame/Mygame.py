...
def game_loop():
    ...
    while not choose:
        ...
        message(font_style, "Left,right or lshift to choose new game level", WHITE, 30, HEIGHT // 2+50)
    ...
    tempFood = [foodx,foody]
    if tempFood in walls2 and lvl ==2:
        while  tempFood in walls2:
            foodx = round(random.randrange(0, WIDTH - snake_block_size) / 10.0) * 10.0
            foody = round(random.randrange(0, HEIGHT - snake_block_size) / 10.0) * 10.0
            tempFood = [foodx,foody]
    elif tempFood in walls3 and lvl ==3:
        while  tempFood in walls3:
            foodx = round(random.randrange(0, WIDTH - snake_block_size) / 10.0) * 10.0
            foody = round(random.randrange(0, HEIGHT - snake_block_size) / 10.0) * 10.0
            tempFood = [foodx,foody]
    ...
    while not game_close:
        ...
        message(score_font, "Your score: " + str(snake2.get_length() - 1), BLUE, 640, 0)
        ...
        while game_over:
            ...
            if snake1.get_length() > snake2.get_length():
                message(font_style, "Black won with score", YELLOW, WIDTH //3+30, HEIGHT // 2)
                message(font_style, ""+ str(snake1.get_length()-1), YELLOW, WIDTH //3+30, HEIGHT // 2+50)
            elif snake1.get_length() < snake2.get_length():
                message(font_style, "White won with score", BLUE, WIDTH //3+30, HEIGHT // 2)
                message(font_style, ""+ str(snake2.get_length()-1), BLUE, WIDTH //3+30, HEIGHT // 2+50)
            else:
                message(font_style, "DRAW", GREEN, WIDTH //3+30, HEIGHT // 2)
